...
\author{Stephan Schulz}
\title{E 1.7\\[1.5ex]User Manual\\[1.5ex]{\normalsize --preliminary
    version--}}
...
\begin{abstract}
  ...
  on superposition and rewriting. In this perpetually very preliminary
  manual we first give a short introduction for impatient new users,
  and then cover calculus and proof procedure. The manual covers proof
  search control and related options, followed by input and output
  formats. Finally, it describes some additional tools that are part
...
This is a short and currently quite sketchy documentation of the E
...
SPASS\index{SPASS}~\cite{WGR96,WABCEKTT:CADE-99}, DISCOUNT~\cite{DKS97},
...
\begin{tabular}{lp{7.5cm}}
  ...
  problem features).\\
  \texttt{--auto}    & As \texttt{--auto}, but add heuristic
...
If you use problems in the current version of the TPTP
language\index{TPTP!language}, you
should also add the option \texttt{--tptp3-format} (or only
\texttt{--tptp3-in} if you like the traditional output more).
...
operators and quantors. This format is reduced to clause normal form
in a way that the CNF is unsatisfiable if and only if the original
problem is provable (if an explicit \emph{conjecture} is given) or
...
\begin{definition}[The inference system \textbf{SP}]
 ...
 \item \emph{Simultaneous superposition into negative
    ...
    \bigskip (SSN) \GInferenz{\eqn{s}{t} \vee S \phantom{ae} \neqn{u}{v} \vee
     R}{\sigma(\neqn{u[u_p\leftarrow t]}{v} \vee S \vee R)}{if
...
\item \emph{Simultaneous superposition into positive literals}
  \index{simultaneous superposition}
   ...
   \bigskip (SSP) \GInferenz{\eqn{s}{t} \vee S \phantom{ae} \eqn{u}{v} \vee
    R}{\sigma(\eqn{u[u_p\leftarrow t]}{v} \vee S \vee R)}{if
    $\sigma=mgu(u|_p,s)$, $\sigma(s)\not<\sigma(t)$,
    $\sigma(u)\not<\sigma(v)$, $\sigma(\eqn{s}{t})$ is eligible for
    paramodulation, $\sigma(\neqn{u}{v})$ is eligible for resolution,
    and $u|_p \notin V$.}
 ...
 \item \emph{Contextual literal cutting}\index{contextual literal cutting}:

   \bigskip (CLC) \CInferenz{\sigma(C \vee R \vee \ueqn{s}{t})
     \hspace{2em} C \vee \overline{\ueqn{s}{t}}}{\sigma(C \vee R)
     \hspace{5em} C \vee \overline{\ueqn{s}{t}}}
   {where $\overline{\ueqn{s}{t}}$ is the negation of $\ueqn{s}{t}$
     and $\sigma$ is a substitution} 
...
\item \emph{Condensing}\index{condensing}:
  ...
  $\sigma(l_1)\equiv\sigma(l_2)$ and $\sigma(l_1 \vee R)$ subsumes
  $l_1 \vee l_2 \vee R$}



 \item \emph{Introduce definition\index{clause
       splitting}}\footnote{This rule is always exhaustively aplied to
...
\item Classical \emph{relevancy pruning}\index{relevancy pruning}
  ...
  axioms that shares such a symbol is considered relevant. Symbols in
  selected axioms become relevant, and so become all their function
  symbols. The process is then repeated for a selected number of
  iterations. The option \texttt{--rel-pruning-level} determines how
  many iterations are performed.    Relevance pruning is complete in the
  non-equational case if allowed to reach a fixed point. It only
  provides a relatively coarse measure, however.
...
\end{verbatim}
  ...
  \item \texttt{<g-measure>} is the generality measure. Currently,
    \texttt{CountFormulas} and \texttt{CountTerms} are supported.
...
E natively uses E-LOP, a dialect of the LOP\index{LOP} language designed for
SETHEO\index{SETHEO}. At the moment, your best bet is to retrieve the
LOP description from the E web site~\cite{E:WWW-04} and/or check out
the examples available from it.  LOP is very close to Prolog, and E
can usually read many fully declarative Prolog files if they do not
use arithmetic or rely on predefined symbols. Plain SETHEO files
usually also work very well.  There are a couple of minor differences,
however:
...
\item E does not support constraints or SETHEO\index{SETHEO} build-in
...
TPTP syntax version 3~\cite{SSCG:IJCAR-2006} is the currently
recommended format. It is supported by many provers, it is more
consistent than the old TPTP language, and it adds a number of useful
features.  E supports TPTP-3 syntax with the options
...
\section{Impressing your Friends}
...
inferences, and at level 6 it also prints PCL steps giving a lot of
insight into the internal operation of the inference engine.  This
protocol is fairly readable and, from level 4 on can be used to check
the proof with the utility \texttt{checkproof} provided with the
distribution.
...
E is designed to handle individual proof problems, one at a time. For
this case, the prover has mechanism to handle even large
specifications. However, in cases where multiple queries or
conjectures are posed against a large background theory, even the
parsing of the background theory may take up significant
time. \texttt{E\_ltb\_runner} has been developed to efficiently handle
this situation. It can read the background theory once, and then run E
with additional axioms and different conjectures against this
background theory.
...
supports interactive queries. However, the program is in a more
prototypical state than most of the E distribution. In particular, any
syntax error in the input will cause the whole program to terminate.

By default, it will process a batch specification file
(see~\ref{sec:bsf}), which contains a specification of the background
theory, some options, and (optionally) a number of individual job
requests. If used with the option \texttt{--interactive}, it will
enter interactive mode (\ref{sec:iq}) after all batch jobs have been
processed.
...
syntax. The main program runs both \texttt{eprover} and
\texttt{epclextract} as sub-processes. Unless these programs are in the
search path, their full paths should be given as optional arguments.
...
year. E tracks this development. The E~1.7 distribution implements
support for the LTB format used in CASC-J6 and documented at
\url{http://www.cs.miami.edu/~tptp/CASC/J6/Design.html#Problems},
subsection \emph{Batch Specification Files}.
...
\newblock In Nikolaj Bj{\o}rner and Sofronie-Viorica Stokkermans, editors, {\em
...
\bibitem[Sch12]{E:WWW-04}
S.~Schulz.
\newblock {The E Web Site}.
\newblock \url{http://www.eprover.org}, 2004--2012.
