...
\author{Stephan Schulz}
\title{E 2.2\\[1.5ex]User Manual\\[1.5ex]{\normalsize --preliminary
    version--}}
...
prover supports efficient \index{propositional reasoning} by
integrating \index{PicoSAT}~\cite{Biere:JSBC-2008}. The prover
periodically grounds all clauses in the proof state and encodes the
set of ground clauses for the propositional prover. If the
propositional prover finds a proof for unsatisfiability of the ground
problem, Herbrand's theorem allow us to lift this to the first-order
level. In the implementation, the prover extract the unsatisfiable
core of the grounded clause set, retrieves the corresponding
first-order clauses, and adds an suitable pseudo-inference to the
first-order proof objects. A more detailed description is available
...
(\url{http://matryoshka.gforge.inria.fr/}), since version 2.3
E has been extended to support a fragment of higher-order 
logic called lambda-free higher-order logic (LFHOL). 
In this section we will give basics needed to understand
LFHOL syntax. Detailed description (which includes semantics) is given
in paper by Bentkamp et al. 
(\url{http://matryoshka.gforge.inria.fr/pubs/lfhosup_report.pdf}). 
...
there are no comprehension principles and boolean formulas can appear
only as atoms (not as arguments to other symbols). Furthermore,
quantification over booleans is not allowed. LFHOL is simply typed -- 
each type is either atomic or function type.

In E 2.3, LFHOL formulas are input using a subset of 
TPTP THF syntax. More precisely, E 2.3 supports subset of THF syntax 
that is monomorphic, without lambda abstractions, constants !! and ??
and anything else that you get an error for :)

For example, LFHOL is expressive enough to reason about higher-order
combinators such as \textsf{map} (pointwise application of a function to list)
or \textsf{power} (iterative oplication of a function). 
Problem \texttt{list.p} in folder \texttt{EXAMPLE\_PROBLEMS/LFHOL} 
shows all LFHOL features used to succintly
express problem that has more verbose encodings in FOL. 
...
In most HOLs, this formula is provable, and the witness is function
$\lambda x \, y. \, \textsf{f} \, y \, x$. In LFHOL, without axiomatizing
the function that permutes \textsf{f}'s arguments, the formula is not
provable. This problem (both with and without appropriate axioms)
is present in the folder \texttt{EXAMPLE\_PROBLEMS/LFHOL}.
