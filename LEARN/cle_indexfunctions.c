...
void TSMIndexPrint(FILE* out, TSMIndex_p index, int depth)
{
   ...
   {
   case IndexArity:
         locked_fprintf(out, "# %sIndex %ld is arity index!\n", pattern2, index->ident);
         break;
   case IndexSymbol:
         locked_fprintf(out, "# %sIndex %ld is symbol index!\n",
                 pattern2,index->ident);
         locked_fprintf(out, "# %sPSymbol         Index  FCode     (Symbol)\n", pattern2);
         ...
         while((ncell = NumTreeTraverseNext(stack)))
         {
            ...
            locked_fprintf(out, "# %s#%10ld :%7ld  %7ld     %s\n", pattern2,
                    ncell->key,
                    ncell->val1.i_val,
                    f_code,
         ...
         locked_fprintf(out, "# %s%ld alternatives in the index\n",
                 pattern2, i);
         break;
   case IndexTop:
   case IndexAltTop:
   ...
   case IndexESTop:
   case IndexIdentity:
         locked_fprintf(out, "# Index is %s index!\n",
         ...
         {
            current = pcell->key;
            locked_fprintf(out, "# %3ld : ", current->key);
         ...
         PTreeTraverseExit(stack);
         locked_fprintf(out, "# %ld alternatives in the index\n", i);
         break;
   case IndexEmpty:
         locked_fprintf(out, "# Index is empty index!\n");
         break;
   default:
         assert(false && "Unknown index type");
         break;
