                                              ...
                                              RewriteLevel level)
{
   ...
   if(control->heuristic_parms.enable_given_forward_simpl)
   {
      ...
      pclause = FVIndexPackClause(clause, state->processed_non_units->efficient_subsumption_index->fvindex);
      ...
      if(context_sr && ClauseLiteralNumber(clause) > 1)
      {
         ...
         pclause = FVIndexPackClause(FVUnpackClause(pclause),
                                state->processed_non_units->efficient_subsumption_index->fvindex);
