...
double ProofWeightCompute(void* data, Clause_p clause)
{
   ...
   res = ClauseWeight(clause, 
		      ...
		      local->fweight, 
		      false);
   res *= (1+local->proof_depth_multiplier*(1L/(clause->proof_depth+1)));
   res *= (1+local->proof_size_multiplier*(1L/(clause->proof_size+1)));
   return res;
}
